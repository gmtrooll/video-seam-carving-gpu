...
int main(int argc, char* argv[]) {
    ...
    maxFrames = cap.get(CAP_PROP_FRAME_COUNT);
    int origWid = cap.get(CAP_PROP_FRAME_WIDTH);
    int origHei = cap.get(CAP_PROP_FRAME_HEIGHT);
    
    Size S = Size((int)cap.get(CAP_PROP_FRAME_WIDTH) -ver , (int)cap.get(CAP_PROP_FRAME_HEIGHT)-hor);

    string::size_type pAt = inFile.find_last_of('.');   // Find extension point
    const string outFile = inFile.substr(0, pAt) + "-result.mov";
    output.open(outFile, VideoWriter::fourcc('m','p','4','v'), cap.get(CAP_PROP_FPS), S, true);
...
Mat reduce_frame(Mat frame1, Mat frame2, Mat frame3, Mat frame4, Mat frame5, int v, int h) {
    ...
    cvtColor(frame1,grayImg1, COLOR_RGB2GRAY);
    cvtColor(frame2,grayImg2, COLOR_RGB2GRAY);
    cvtColor(frame3,grayImg3, COLOR_RGB2GRAY);
    cvtColor(frame4,grayImg4, COLOR_RGB2GRAY);
    cvtColor(frame5,grayImg5, COLOR_RGB2GRAY);
